...
为了评估 SparkBWA 在分布式环境中的性能表现，我们设置 2 个主要的对比基准。

第一个是 使用单机环境使用 bwa-0.7.9 对 hg38 索引文件数据进行序列对比，采用 8 线程对比。
第二个是 在分布式环境下使用 SparkBWA 在三节点服务器上，同样适用 hg38 索引文件进行序列对比，设置运行的 container 数量为 3， 调用 bwa 时候同样使用 8 线程。

对比的指标为程序运行时间。来评估分布式并行对性能的影响。

为了更方便地进行基准测试，我们使用了一个[脚本](https://github.com/chengkaiyang2025/SparkBWA/blob/7d89d26edfbb07c5e97811c0207b6b489fb7db74/submit_hg38.sh)分别进行调度，并分别统计运行时间。
# 6. Results & Performance Analysis
## 6.1 实验结果概述

在使用同样的配置反复实验后，实验结果表明，相比于单机运行的 BWA，SparkBWA 在多节点环境下能够显著缩短整体运行时间。在当前实验规模下，SparkBWA 在三台服务器上的总执行时间大约为单机 BWA 的 1/2 到 1/3 之间，也就是
获得了 2 倍 甚至 3 倍的性能提升。

部分实验的日志[可以从 github repo](https://github.com/chengkaiyang2025/SparkBWA/tree/spark3/logs) 中获得。
## 6.2 单机 BWA 与 SparkBWA 的性能对比与分析
由于我们把控制了 container 数量为 3，我们期望 3 个 container 能够分别在 3 个服务器上并行运行。但由于 yarn 的资源管理调度配置，有的时候 2 个 container 会在同一台服务器上运行，导致吞吐量不一定能达到单机运行
bwa 的 3 倍以上。

而且我们设置了失败重试机制，因此有时 SparkBWA 在第一次尝试运行时候，由于内存资源或其他原因会失败，导致重新运行。我们将第一次尝试运行的时间也加入了统计时间中，因此也会让 SparkBWA 的处理时间达不到 单机 bwa 的 1/3。

但是总体来说，使用 SparkBWA 确实能够一定程度上提升 bwa 的运行速度，更高效地完成序列对比。

## 6.3 实验结果的局限性

即使我们验证了 SparkBWA 能在一定程度上提升 bwa 的运行速度，但我们很难在本次实验中具体量化相比于 bwa 具体提升了多少倍的性能。原因有以下几点：
1. 每个 container 所需要的内存达到 8GB 左右，而每个节点最多分配的内存为 16GB，有的时候当 2 个container 分配到同一个节点上，会有可能因为内存不够而导致任务挂掉重试。
2. 难以找到一个最佳配置，量化 SparkBWA 的性能提升倍数。这是因为我们必须考虑到索引文件的大小，以及序列文件的大小，以及每个节点最多能跑几个 container。如果我们换一个索引文件，例如 chr22 ，那么所有的配置还可以调优，
因为 chr22 的索引文件比 hg38 要小，我们可以设置更小的堆外内存，更多的 container 来进行测试。
